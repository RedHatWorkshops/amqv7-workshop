# Addressing in AMQ7 broker

This worksheet covers the addressing model in the AMQ7 broker.  By the end of this worksheet you should know:

1. The core addressing concepts in AMQ7 broker
    * How routing works within the AMQ7 broker
    * How protocol managers implemented various address semantics using the AMQ7 Core API
...
## AMQ7 Addressing Core Concepts
There are 3 main core concepts within the AMQ7 broker which are responsible for a variety of addressing behaviours within the AMQ7 broker.  The 3 main concepts are as follows:

1. Queues: Contain a unique name, a routing type and an ordered list of messages.  (As well as some more advanced settings which we will touch on later).
2. Addresses: Contains a string address, a list of queues and a set of enabled routing types.
3. Routing Type: Either ANYCAST or MULTICAST.  Defines how messages are routed from addresses to queues.

Note: There is no such thing as a Topic in AMQ7.  There are only addresses, queues and routing types.  We'll show later in this worksheet how publish subscribe works within AMQ7.

### Routing messages to queues
When AMQ7 receives a message from a publisher (in any protocol), the protocol manager, from which the publisher is speaking to, will interact with the AMQ7 core API and will route the message to a zero or more queues.  We can think of this routing as **inbound** routing, from addresses to queues.  The **inbound** routing logic works as follows:

1. For the message address, find all the addresses that match (some addresses may have wildcards and therefore there could be more than 1).
2. For each address, route a copy of the message to **all** of the associated queues with a "MULTICAST" routing type and route a copy of the message to **one** of the "ANYCAST" queues.

Note.  Queue filtering is also done on inbound routing.  Only messages that match the address and have an appropriate filter will recieve a copy of the message.

Note.  There is only ever one copy of the message stored with in the AMQ7 broker.  Actually what is added to the queue is a pointer to the original message (known internally as a message reference).  AMQ7 broker uses reference counting to know when it is safe to delete a message.
...
Leave the broker running and open up the broker.xml. Add the following lines to the addresses section:
...
AMQ7 broker can pick up new addresses and queues from configuration at runtime.  Go the the HawtIO console and verify that the addresses and queues were successfully created.
...
Go back to the HawtIO console, navigate to the each of the queues we created and browse their messages.  Notice that sub1 and sub2 have a copy of the message.  Have a play around to see how various configuratins behave.  Try:

1. Adding more than one ANYCAST queue is added to an address.
2. Add both ANYCAST queues and MULTICAST queues to the same address.
...
#### Outbound Routing
Before we move on, it's important to introduce one more component of the AMQ7 Core API.  The server consumer.  The server consumer subscribes to a single queue within the broker.  One queue may have more that one server consumer.  The AMQ7 broker takes care of distributing messages from queues to server consumers.  For the sake of this tutorial we'll call the routing of messages from queues to consumers **outbound** routing.  We'll take a look at how these internal consumers play a role within the overall AMQ7 broker picture.

## Core Addressing Wildcard Syntax
The AMQ7 broker offers a wildcard syntax.  Wildcards can be used in address names and are used to match against received messages.  
...
## Protocol managers and the AMQ7 broker Core API

AS mentioned earlier, protocol managers map protocol specific semantics onto the AMQ7 broker Core API using the three main concepts, addresses, queues, routing types explained earlier.  This section will give a high level overview of how various addressing semantics are acheived using AMQ7 broker Core API.  This aim of this section is to give you an insight as to what is happening under the covers and also to lay the foundation for later chapters.
...
Note: AMQ7 broker can automatically create queues and addresses on demand.  Autocreation is enabled by default out of the box.
...
Change the following lines under the <address-settings name="#" ..> section of the broker.xml
...
### JMS Topics
To create the equivilent of a JMS Topic in the AMQ7 broker, we need to create an address with the MULTICAST routing type enabled.  To do this add the following XML snippet to the broker.xml
...
#### subscription
As mentioned earlier, there is no such thing as a topic in the AMQ7 broker, there are only the 4 key concepts we've spoken about in this tutorial.  Subscriptions are implemented by the way of "subscription queues".  Subscription queues have the MULTICAST routing type, which tells the AMQ7 broker that this queue requires a copy of every message that matches the queues address and a bunch of attributes that define some additional semantics.
...
This concludes the section on Protocol managers and the AMQ7 broker Core API.  In summary.  There are 4 main concepts that lay the foundation for all addressing semantics in AMQ7 broker.  Addresses, queues, routing types and server consumers.  Protcol managers control these objects in order to provide protocol specific sematnics to clients.  ANYCAST addresses are akin to JMS Queues (or point to point) MULTICAST addresses are akin to JMS Topics.
...
## Defining Address Sematnics in Broker Configuration
